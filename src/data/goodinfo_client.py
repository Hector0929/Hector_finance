"""
goodinfo_client.py — 從 Goodinfo.tw 抓取台灣股票財報數據
含三層驗證機制（Provenance / Sanity / MOPS連結）
結果會快取至 Firebase Firestore（若已設定）
"""
...
def _compute_metrics(is_d: dict, bs_d: dict, years: list) -> dict:
    """從損益表 + 資產負債表計算衍生財務指標。"""
    def g(table, key, yr):
    ...
    for yr in years:
        ...
        op_key  = next((k for k in is_d if '營業利益' in k and '損失' not in k), None)
...
def get_financial_analysis(stock_id: str, use_cache: bool = True) -> dict:
    """
    主要對外介面：取得個股完整財務分析資料。

    優先從 Firebase 快取讀取（TTL 24 小時）。
    快取不存在或過期時，從 Goodinfo.tw 抓取並更新快取。

    回傳 dict 包含：
        stock_id, years, income_statement, balance_sheet, cash_flow,
        metrics (衍生指標), metadata, verification
    失敗時回傳含 error 欄位的 dict。
    """
    try:
        from src.data.firebase_client import get_financials_cache, save_financials_cache
        firebase_available = True
    except ImportError:
        firebase_available = False

    if use_cache and firebase_available:
        cached = get_financials_cache(stock_id)
        if cached is not None:
            logger.info(f"財報快取命中 stock_id={stock_id}")
            return cached

    try:
        data = fetch_all(stock_id)
        years = data['years'][:3]
        metrics = _compute_metrics(data['income_statement'], data['balance_sheet'], years)
        data['metrics'] = metrics
        data = run_verification(data, metrics)

        if firebase_available:
            save_financials_cache(stock_id, data)

        return data
    except Exception as e:
        logger.error(f"get_financial_analysis 失敗 stock_id={stock_id}：{e}")
        return {"error": str(e), "stock_id": stock_id}


if __name__ == '__main__':
    stock_id = sys.argv[1] if len(sys.argv) > 1 else '2330'
    result = get_financial_analysis(stock_id, use_cache=False)
